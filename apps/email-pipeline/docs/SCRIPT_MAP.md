...
| `scripts/commercial/build_commercial_procurement_read_model.py` | OPS_AUDIT / dry-run | **PR4** deterministic procurement planner (`commercial_procurement_*` plan only); requires `--sqlite-path`, `--as-of-date`, `--run-context`; dry-run only (`--apply` refused); `mode=ro` + `query_only`; never creates production tables; never mutates PR2/PR3/Gmail/Postgres — see [`COMMERCIAL_PROCUREMENT_LINK_READ_MODEL_PR4.md`](audits/COMMERCIAL_PROCUREMENT_LINK_READ_MODEL_PR4.md) |
...
| `scripts/commercial/build_commercial_procurement_read_model.py` | **`--apply` refused** in planner/dry-run checkpoint (no production DDL; plan-only) |
