...
| `scripts/qa/audit_commercial_procurement_link.py` | OPS_AUDIT | **Read-only** PR4 procurement ↔ PR2 account-link audit: requires `--sqlite-path` + `--output-dir`; `mode=ro` + `query_only`; no table writes; gitignored reports — see [`COMMERCIAL_PROCUREMENT_LINK_READ_MODEL_PR4.md`](audits/COMMERCIAL_PROCUREMENT_LINK_READ_MODEL_PR4.md) |
