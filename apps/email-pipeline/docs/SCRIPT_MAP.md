...
| `scripts/commercial/build_commercial_identity_read_model.py` | OPS_MAINT / break-glass | **PR2** deterministic account/contact identity read model (`commercial_identity_*`); requires `--sqlite-path`; dry-run default; `--apply` uses **transaction contract B** (additive schema may remain; DELETE+INSERT data replacement is atomic with `foreign_keys=ON`) — **not** production without approval; see [`COMMERCIAL_IDENTITY_READ_MODEL_PR2.md`](audits/COMMERCIAL_IDENTITY_READ_MODEL_PR2.md) |
