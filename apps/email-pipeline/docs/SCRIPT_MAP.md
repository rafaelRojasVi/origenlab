...
| `scripts/migration/import_waves_into_v2.py` | import_ingest | V2 historical import (Wave 1A/1B → V2) | the immutable Wave 1A bundle, its RFC 2047 addendum, the Wave 1B bundle, the cross-wave reconciliation report | **nothing by default**; with `--apply`, `evidence.*` and `outbound.contact_control` on a **disposable loopback** PostgreSQL only | medium | `uv run python scripts/migration/import_waves_into_v2.py --migration-root … [--apply --database-url postgresql://…@127.0.0.1:…/…]` | **Dry-run by default — no connection is opened without `--apply`.** Refuses any non-loopback or hosted DSN, with no override flag. Verifies every artifact hash before parsing a row. One transaction; `ON CONFLICT DO NOTHING`; never deletes, truncates or updates. Never writes `crm.*` — no person, organization, contact point or prospect is created. Aggregate output carries counts only; recipient-level rejects go to a `0600` artifact outside Git. No Gmail, Supabase, Render or Cloudflare call; send flags untouched |
