...
| `scripts/maintenance/rehearse_sqlite_writable_restore.py` | OPS_MAINT | **Synthetic writable-restore rehearsal** only (tiny throwaway DBs); refuses production and real compact candidates; default preflight; `--confirm-throwaway --apply` for controlled writable txn + rollback — see [`SQLITE_WRITABLE_RESTORE_AND_CUTOVER.md`](SQLITE_WRITABLE_RESTORE_AND_CUTOVER.md) |
