...
| `scripts/qa/audit_sqlite_deep.py` | OPS_AUDIT | Read-only **deep forensic** audit for verified offline copies only; structural integrity, dbstat attribution, body-column byte totals, duplicate/usefulness classification; heavy phases require `--confirm-offline-copy` and refuse configured production SQLite — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
