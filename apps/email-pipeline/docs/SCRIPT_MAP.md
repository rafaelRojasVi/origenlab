...
| `scripts/maintenance/backup_sqlite_online.py` | OPS_MAINT | **Online Backup API** snapshot via `Connection.backup()` (never `cp`/`rsync`); explicit `--source`/`--destination`; `.partial` + atomic rename; same-FS refused by default; capacity check; sanitized manifest; planned dest `/mnt/d/origenlab-sqlite-offline` — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
