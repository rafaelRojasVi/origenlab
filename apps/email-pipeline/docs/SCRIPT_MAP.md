...
Last reviewed: 2026-08-07 (docs maintenance — equipment direct-publish runbooks + commercial stack map through PR5E.2; preserve SCRIPT_MAP commercial rows from #434/#436/#437)
...
| `scripts/qa/audit_email_mart_feature_scan.py` | superseded_by_origenlab | `origenlab audit-email-mart-feature-scan` | SQLite emails + features | — | low | `uv run origenlab audit-email-mart-feature-scan` | **Read-only** parity audit; no mart table writes |
...
| `scripts/mart/build_email_mart_features.py` | superseded_by_origenlab | `origenlab build-email-mart-features` | SQLite raw | SQLite `email_mart_features` | medium | `uv run origenlab build-email-mart-features` | **Dry-run** default; **`--apply`** writes missing/stale rows; **`--missing-only`** inserts new rows only; **`daily-core --apply`** runs missing-only apply before feature-backed mart rebuild |
...
| `scripts/campaigns/outbound_campaign_cli.py` | superseded_by_origenlab | `origenlab outbound-campaign` | SQLite `outbound_campaign*` / `manual_contact_status` | SQLite (`export` subcommand: explicit `--out` CSV/JSON only) | **high** | `uv run origenlab outbound-campaign -- status --campaign-id <id>` | Durable campaign ledger; `send` dry-run default, `--live` calls Gmail API (break-glass) |
...
| *(orchestrator)* | active_operator_command | `origenlab auto-refresh-chilecompra-equipment` | Mercado Público API + equipment queue builders + optional Postgres direct publish | state file + optional Postgres | **high** | `uv run origenlab auto-refresh-chilecompra-equipment --once` | Preferred live equipment refresh; `--apply --publish-read-model` publishes to `api.v_equipment_opportunity_current` (PHASE W1 2026-08: legacy/manual-backfill opt-in, default `false`; tracked wrapper disables it) — [`operator/CHILECOMPRA_EQUIPMENT_REFRESH.md`](operator/CHILECOMPRA_EQUIPMENT_REFRESH.md) |
| *(orchestrator)* | read_only_qa_report | `origenlab operator-automation-status` | local state files + user crontab | — | low | `uv run origenlab operator-automation-status` | Read-only health for loops A+B; `--json`; `--skip-cron-inspection` |
| `scripts/operator/run_auto_refresh_mail.sh` | active_operator_command | cron wrapper | — | optional SQLite via CLI | medium | `scripts/operator/run_auto_refresh_mail.sh` | ~3m cron; calls `auto-refresh-mail --once --apply` — [`pipeline/OPERATOR_CRON.md`](pipeline/OPERATOR_CRON.md) |
| `scripts/operator/run_auto_mirror_dashboard.sh` | active_operator_command | cron wrapper | — | optional Postgres via CLI | **high** | `scripts/operator/run_auto_mirror_dashboard.sh` | ~15m cron; calls `auto-mirror-dashboard --once --apply --allow-non-scratch-postgres` |
...
| `scripts/qa/plan_function_surface.py` | read_only_qa_report | Refactor / cleanup planning | `src/`, `scripts/` | `reports/local` only | low | `uv run python scripts/qa/plan_function_surface.py` | AST + size/risk planner — **not** deletion authority |
| `scripts/qa/plan_import_surface.py` | read_only_qa_report | Refactor / cleanup planning | `src/`, `scripts/`, `tests/`, `docs/` | `reports/local` only | low | `uv run python scripts/qa/plan_import_surface.py` | Import/reference planner — **not** deletion authority |
| `scripts/reports/generate_client_report.py` | read_only_qa_report | Client pack / PDF reports | SQLite mart | `reports/out` | low | `uv run python scripts/reports/generate_client_report.py` | Large report generator — not send path |
...
| `scripts/migration/extract_wave1b_v1_safety_bundle.py` | read_only_qa_report | V2 migration evidence (Wave 1B) | SQLite `mode=ro` + verified `PRAGMA query_only=ON` | operator-named dir **outside** the repo | low | `uv run python scripts/migration/extract_wave1b_v1_safety_bundle.py --sqlite-path … --output-dir … --wave1a-snapshot-at …` | One deferred read transaction; never touches the ingest cron; no Gmail call; fails closed on schema/state/reference/count/Sent-history drift; exports no operator free text; artifacts are owner-only |
| `scripts/migration/derive_wave1a_rfc2047_addendum.py` | read_only_qa_report | V2 migration evidence (Wave 1A addendum) | the immutable Wave 1A bundle | operator-named dir **outside** the repo | low | `uv run python scripts/migration/derive_wave1a_rfc2047_addendum.py --bundle-dir … --output-dir …` | Never edits, regenerates or relabels the Wave 1A bundle |
| `scripts/migration/reconcile_wave1a_wave1b_safety.py` | read_only_qa_report | V2 migration evidence (cross-wave safety baseline) | the immutable Wave 1A bundle, its RFC 2047 addendum, the Wave 1B bundle | operator-named dir **outside** the repo | low | `uv run python scripts/migration/reconcile_wave1a_wave1b_safety.py --wave1a-dir … --wave1a-addendum … --wave1b-dir … --output-dir …` | Files only — no database, mailbox or hosted service; verifies every checksum first; measures the overlap instead of summing the two waves; report carries counts, never addresses |
| `scripts/migration/harden_wave1a_artifact_permissions.py` | read_only_qa_report | V2 migration evidence (artifact privacy) | the immutable Wave 1A bundle, its archive and sidecar | **filesystem modes only**, inside the named boundary | low | `uv run python scripts/migration/harden_wave1a_artifact_permissions.py --bundle-dir … --archive … [--dry-run]` | The only tool that chmods files it did not create; verifies every checksum before and after and refuses if a digest moves; never touches the enclosing root or anything outside the named boundary; writes, renames and reads no content |
| `scripts/migration/import_waves_into_v2.py` | import_ingest | V2 historical import (Wave 1A/1B → V2) | the immutable Wave 1A bundle, its RFC 2047 addendum, the Wave 1B bundle, the cross-wave reconciliation report | **nothing by default**; with `--apply`, `evidence.*`, `outbound.*` and `comms.mailbox` on a **disposable loopback** PostgreSQL only | medium | `uv run python scripts/migration/import_waves_into_v2.py --migration-root … [--apply --database-url postgresql://…@127.0.0.1:…/…]` | **Dry-run by default — no connection is opened without `--apply`.** Accepts a literal loopback address only; a URI query string, fragment, multi-host list, Unix socket, host *name*, hosted marker or host-less DSN all refuse, with no override flag, and the libpq `PG*` environment is removed for the connection. Verifies every artifact hash before parsing a row. Writes under `set local role origenlab_owner`. One transaction; idempotency is provenance-scoped — an existing row is read back and compared before reuse, and a mismatch refuses; never deletes, truncates or updates. Never writes `crm.*` — no person, organization, contact point or prospect is created. Aggregate output carries counts only; recipient-level rejects go to a `0600` artifact outside Git. No Gmail, Supabase, Render or Cloudflare call; send flags untouched |
| `scripts/migration/promote_evidence_into_crm.py` | import_ingest | V2 evidence → CRM promotion (slice 2 identity pass) | `evidence.assertion` rows already loaded in a local V2 database | **nothing by default**; with `--apply`, `crm.organization`, `crm.contact_point`, `crm.domain_event` and the `resolution` of `evidence.assertion`, on a **loopback** PostgreSQL only | medium | `uv run python scripts/migration/promote_evidence_into_crm.py --database-url postgresql://…@127.0.0.1:…/… [--apply] [--link-marketing]` | **Dry-run by default.** `--link-marketing` additionally fills `outbound.campaign_recipient.contact_point_id` from the promoted channels: that column only, never `person_id` or `organization_id`, never `outbound.contact_control` (a suppression is a fact about an address), and no domain event (the join is deterministic and re-derivable, not a human decision). Idempotent — a recipient already linked is never re-linked, so an operator's manual link survives. Uses the importer's loopback target guard unchanged. **Never creates a `crm.person`** — the evidence records no display name for anybody, and the count is asserted zero inside the transaction. **Never attaches a contact point to an organization** — a domain is never an identity key (DOMAIN.md §2.2), also asserted inside the transaction. Never merges two organizations or two people: similar names go to review together. Creates no consent, permission or subscription fact. Identifiers are deterministic UUIDv5 of the evidence, so a re-run and a later hosted replay mint the same ids. Everything created is `machine_proposed`; every event carries `actor_kind = 'migrator'`. One transaction; never updates or deletes a `crm` row. Aggregate output carries counts only — no address, person or organization name is printed. No Gmail, Supabase, Render or Cloudflare call; send flags untouched |
...
|--------|------------|----------------|
| **Core** | Policy and business logic: gate, CSV contracts, outbound preflight, state, suppressions, Gmail helpers | `src/origenlab_email_pipeline/` (Python package) — **not** run directly; imported by scripts and tests. [`src/origenlab_email_pipeline/core/`](../src/origenlab_email_pipeline/core/) holds shared infrastructure implementations only (`safety`, `step_runner`, `reports_out`, `research_automation`, `core.mart`, `core.outbound` builders); the former Stage 2A/2B re-export facade layer was removed in the 2026-08 commercial platform reset — import implementations from the top-level modules. |
...
| `scripts/qa/audit_mailbox_intake_inventory.py` | OPS_AUDIT | Read-only identity-lane × intake-class inventory of `emails`/`email_mart_features` (`mode=ro` + `query_only`); aggregate counts only — emits no address, subject or message-id; brackets its own read and reports `stable_snapshot=false` if the cron ingest wrote mid-audit |
...
| `scripts/qa/audit_sqlite_storage.py` | OPS_AUDIT | Read-only SQLite size/freelist (+ Sent diagnostics by default); `--storage-only` for constant-time metadata; optional expensive `--include-dbstat`; **never VACUUM** — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
| `scripts/qa/audit_sqlite_deep.py` | OPS_AUDIT | Read-only **deep forensic** audit; production `--light-only` = constant-time PRAGMAs + schema inventory; heavy phases (quick_check/FK/COUNTs, dbstat, body bytes, SHA-256 dupes, usefulness) require `--confirm-offline-copy` and refuse configured production SQLite; `--full-integrity-check` opt-in only — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
| `scripts/maintenance/backup_sqlite_online.py` | OPS_MAINT | **Online Backup API** via `Connection.backup()` (never `cp`/`rsync`); default **preflight** (zero writes); `--apply` required to write; `.partial` + final manifest completion marker; same-FS refused by default; capacity check; orphan detection; planned dest `/mnt/d/origenlab-sqlite-offline` — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
| `scripts/maintenance/compact_sqlite_offline.py` | OPS_MAINT | **Offline `VACUUM INTO` candidate** from verified snapshot; default preflight; requires `--confirm-offline-copy --apply` to write; refuses production; immutable source; no-clobber publish; never in-place VACUUM / never production swap / never deletes source — see [`SQLITE_STORAGE_MAINTENANCE.md`](SQLITE_STORAGE_MAINTENANCE.md) |
| `scripts/maintenance/run_sqlite_maintenance_with_exit_marker.sh` | OPS_MAINT | Wrapper that always records numeric `AUDIT_RESUME_EXIT=<rc>` (including `0`) for systemd-supervised offline jobs |
| `scripts/maintenance/rehearse_sqlite_writable_restore.py` | OPS_MAINT | **Synthetic writable-restore rehearsal** only (scratch-root + marker + ≤64 MiB); operations `preflight` (zero-write) / `build-fixture --apply` / `rehearse --apply`; refuses production and offline/compact shapes — see [`SQLITE_WRITABLE_RESTORE_AND_CUTOVER.md`](SQLITE_WRITABLE_RESTORE_AND_CUTOVER.md) |
| `scripts/maintenance/orchestrate_sqlite_production_cutover.py` | OPS_MAINT | **Staged production cutover orchestrator** (fail-closed); default `plan_preflight` is zero-write; each stage needs its own `--apply` + production auth; `renameat2(RENAME_EXCHANGE)` only — see [`SQLITE_PRODUCTION_CUTOVER_ORCHESTRATOR.md`](SQLITE_PRODUCTION_CUTOVER_ORCHESTRATOR.md) |
...
| `scripts/qa/audit_commercial_truth.py` | OPS_AUDIT | **Read-only** commercial truth audit (PR1): requires `--sqlite-path` + `--output-dir`; no production fallback; gitignored reports|
| `scripts/qa/audit_commercial_procurement_link.py` | OPS_AUDIT | **Read-only** PR4 procurement ↔ PR2 account-link audit: requires `--sqlite-path`, `--output-dir`, and `--as-of-date YYYY-MM-DD`; `mode=ro` + `query_only`; no table writes; no Gmail, Postgres, suppression, outreach, classification, or deal mutation; gitignored reports|
| `scripts/commercial/build_commercial_identity_read_model.py` | OPS_MAINT / break-glass | **PR2** deterministic account/contact identity read model (`commercial_identity_*`); requires `--sqlite-path`; dry-run default; `--run-context` metadata label; `--apply` uses **transaction contract B** (additive schema may remain; DELETE+INSERT data replacement is atomic with `foreign_keys=ON`); stores `identity_fingerprint` for PR3 gating — **not** production without approval; also reachable via `uv run origenlab build-commercial-identity` (adds a durable JSONL run record)|
| `scripts/commercial/build_commercial_opportunity_read_model.py` | OPS_MAINT / break-glass | **PR3** deterministic opportunity-stage read model (`commercial_opportunity_*`); requires `--sqlite-path`; dry-run default (in-memory PR2 identity); `--apply` requires matching persisted PR2 identity fingerprint; contract B; does not mutate deals/identity — **not** production without approval; also reachable via `uv run origenlab build-commercial-opportunity`, or sequenced with PR2 via `refresh-commercial-opportunity-models` (fail-fast, durable JSONL record)|
| `scripts/commercial/build_commercial_procurement_read_model.py` | OPS_AUDIT / apply | **PR4** deterministic procurement planner + transactional persistence (`commercial_procurement_*`); requires `--sqlite-path`, `--as-of-date`, `--run-context`; **default dry-run** (`mode=ro` + `query_only`, no production DDL); `--apply` under contract `B_schema_additive_data_atomic` with stale-plan checks; `production_apply` requires four `--expected-*` fingerprints/digests from an approved dry-run; never mutates PR2/PR3/Gmail/Postgres|
| `scripts/commercial/generate_commercial_procurement_data_walkthrough.py` | OPS_AUDIT | **PR4** redacted procurement data walkthrough generator (markdown + JSON); requires `--sqlite-path` and `--as-of-date`; production SQLite opens `mode=ro` + `query_only`; no production `--apply`; gitignored JSON output|
| `scripts/commercial/build_commercial_procurement_acquisition_snapshot.py` | OPS_AUDIT | **PR5B** fixture/offline acquisition snapshot builder (ticket summary/detail + OCDS parsers); **no network**, rejects `--apply`/`--network`/`--ticket`; never mutates SQLite; digests + walkthrough|
| `scripts/commercial/build_commercial_procurement_candidate_plan.py` | OPS_AUDIT | **PR5C coalescence/lifecycle** read-only planner: PR4 plane + AcquisitionSnapshot JSON → coalesced tenders, unresolved evidence, conflicts, fingerprints, walkthrough; requires `--sqlite-path`, repeatable `--acquisition-snapshot-json`, `--as-of-utc`, `--out-dir`, `--run-context`; SQLite `mode=ro` + `query_only`; rejects `--apply`/`--persist`/`--network`/`--ticket`/`--gmail`/`--postgres`/`--outreach`/`--schedule`; **no product relevance**|
| `scripts/commercial/build_commercial_procurement_product_relevance_plan.py` | OPS_AUDIT | **PR5D** read-only product relevance planner: PR5C coalesced tenders + immutable AcquisitionSnapshot artifacts → reason-coded equipment relevance, reconciliation, labeling queue, and walkthrough; rejects `--apply`/`--persist`/`--network`/`--ticket`/`--gmail`/`--postgres`/`--outreach`/`--schedule`; no contact/outreach/persistence|
| `scripts/commercial/build_commercial_procurement_contact_resolution_plan.py` | OPS_AUDIT | **PR5E** read-only organization/contact resolution over PR5C+PR5D; SQLite `mode=ro`; rejects `--apply`/`--persist`/`--network`/`--ticket`/`--gmail`/`--postgres`/`--outreach`/`--send`/`--schedule`/`--label`|
| `scripts/commercial/build_live_commercial_procurement_review.py` | OPS_AUDIT | **PR5B.2** offline bridge: ChileCompra equipment **detail cache** → PR5B snapshots → PR5C/PR5D/PR5E operator review packet; no network; no DB `--apply`; freshness fail-closed (`BLOCKED_STALE_LIVE_FEED`); actionable/outreach authorization remain false|
| `scripts/commercial/build_commercial_procurement_institution_prospect_plan.py` | OPS_AUDIT | **PR5E.1 / PR5E.2** read-only institution prospect intelligence over PR5C/D/E (live + PR4): lifecycle precedence, line-scoped recognition, event families, review-only clusters, separate operator queues; no persistence/outreach; stacked on PR5B.2|
| `scripts/commercial/build_anexo_recognition_comparison.py` | OPS_AUDIT | **ANEXO-E2** read-only baseline-vs-annex equipment recognition comparison over existing local anexo evidence and detail cache; no production PR5D/PR5E decision changes, persistence, network, Gmail, SQLite/Postgres writes, or outreach|
| `scripts/commercial/build_anexo_shadow_prospect_comparison.py` | OPS_AUDIT | **ANEXO-P1** SHADOW-only CURRENT vs annex-augmented prospect/queue delta over frozen annex evidence; no production queue mutation; no persistence/authorization/PR5F|
| `scripts/commercial/build_pr5d_h3_recognition_coverage.py` | OPS_AUDIT | **PR5D-H3** recognition-coverage matrix (canonical class × detector × catalog capability); gitignored review packet under `reports/out/active/current/pr5d_h3_recognition_coverage/`; no catalog/queue mutation|
| `scripts/commercial/validate_live_procurement_source_contracts.py` | OPS_AUDIT | **PR5B.1** bounded read-only live source-contract validation (max 4 Ticket + 4 public OCDS); requires both `--execute-live-contract-validation` and `--confirm-read-only-contract-validation`; ticket only from `CHILECOMPRA_API_TICKET`; rejects `--apply`/`--ticket` CLI/`--persist`; raw captures gitignored|
| `scripts/commercial/audit_commercial_procurement_live_relevance_pr5.py` | OPS_AUDIT | **PR5A** read-only live-relevance design/audit (artifact open classification); no auth ChileCompra calls; no `--apply`|
...
| `scripts/leads/advanced/export_leads_spanish_csvs.py` | CONSOLIDATE / parked | Spanish `_es` CSV helper — **plan-only default** + **`--write-outputs`**; not daily ops / not send approval |
| `scripts/leads/advanced/run_contact_hunt_web_server.py` | CONSOLIDATE / parked | Local CSV server — **localhost default** + **`--lan`**; password required; not operator API |
...
| `scripts/commercial/build_commercial_identity_read_model.py` | **`--apply`** DELETE+rebuild `commercial_identity_*` data (dry-run default; explicit `--sqlite-path`; contract B: additive schema may remain, data replacement atomic + `foreign_keys=ON`) |
| `scripts/commercial/build_commercial_opportunity_read_model.py` | **`--apply`** DELETE+rebuild `commercial_opportunity_*` data (dry-run default; explicit `--sqlite-path`; requires matching PR2 identity fingerprint; contract B; does not mutate `commercial_deal*` / `commercial_identity_*`) |
| `scripts/commercial/build_commercial_procurement_read_model.py` | **`--apply` implemented** (fixture / separately gated production); default remains dry-run; production apply not executed in this PR |
...
- Postgres planning: [`pipeline/POSTGRES_SCHEMA_TARGET_V1.md`](pipeline/POSTGRES_SCHEMA_TARGET_V1.md)
