...
| `outbound_campaign`, `outbound_campaign_recipient`, `outbound_send_attempt`, `manual_contact_status` | [`outbound_campaign_schema.py`](../../src/origenlab_email_pipeline/outbound_campaign_schema.py) `ensure_outbound_campaign_tables` (single schema-owner module for all four tables) | `outbound_campaign_store.py` (campaign/recipient/attempt CRUD, selection, progress), `manual_contact_status.py` (sidecar CRUD) | Durable **campaign operation** state — `outbound_campaign` (one row per campaign, e.g. `hielscher-sonicators-2026`), `outbound_campaign_recipient` (one row per campaign+email, `UNIQUE(campaign_id, email_norm)`, states `candidate→selected/reserved→sent/blocked/bounced/replied/inactive`), `outbound_send_attempt` (**append-only** attempt ledger; idempotency via `UNIQUE(campaign_id, recipient_id, attempt_seq)` plus an application-level "already accepted" guard so a command retry never re-sends). `manual_contact_status` is the operator contact-lifecycle sidecar (`active`/`inactive`/`hold`); `inactive`/`hold` is a hard exact-email block enforced in [`outbound_campaign_gate.py`](../../src/origenlab_email_pipeline/outbound_campaign_gate.py) (wraps `candidate_export_gate.evaluate_export_eligibility` — does not fork gate logic), `active` is informational only and is **not** marketing consent. No `REFERENCES` into `contact_master`/`organization_master`/`lead_master` (rebuildable projections) — joined by normalized email/domain string only, same convention as `outreach_contact_state.lead_id`. CLI: [`scripts/campaigns/outbound_campaign_cli.py`](../../scripts/campaigns/outbound_campaign_cli.py). This is separate from, and does not replace, the Postgres `outbound.outbound_batch` export-audit mirror (`postgres_outbound_audit.py`). |
