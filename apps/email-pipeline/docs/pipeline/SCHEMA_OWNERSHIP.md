...
Package docstrings document each read model: `commercial_identity/__init__.py` (PR2), `commercial_opportunity/__init__.py` (PR3), `commercial_procurement/__init__.py` (PR4).

Later commercial layers (PR5B.2 live-feed bridge, PR5E.1 institution prospects, PR5E.2 recognition) are **artifact / in-memory** audit paths under their packages — they do **not** add default migrate-stack SQLite families here and are **not** outreach authorization or PR5F persistence. See [`SCRIPT_MAP.md`](../SCRIPT_MAP.md) and each package's own docstring.
...
| `outbound_campaign`, `outbound_campaign_recipient`, `outbound_send_attempt`, `manual_contact_status` | [`outbound_campaign_schema.py`](../../src/origenlab_email_pipeline/outbound_campaign_schema.py) `ensure_outbound_campaign_tables` (single schema-owner module for all four tables) | `outbound_campaign_store.py` (campaign/recipient/attempt CRUD, selection, progress), `manual_contact_status.py` (sidecar CRUD) | Durable **campaign operation** state — `outbound_campaign` (one row per campaign, e.g. `hielscher-sonicators-2026`), `outbound_campaign_recipient` (one row per campaign+email, `UNIQUE(campaign_id, email_norm)`, states `candidate→selected/reserved→sent/blocked/bounced/replied/inactive`), `outbound_send_attempt` (attempt ledger; each attempt row is written once via `record_attempt` for dry-run/skip outcomes, or twice for a live send — `begin_live_attempt` inserts+commits an `in_flight` row *before* the Gmail API call, `finish_live_attempt` resolves it to `accepted`/`failed` after; a crash between the two leaves the row `in_flight`, which a retry must see and refuse to resend. Idempotency via `UNIQUE(campaign_id, recipient_id, attempt_seq)` plus an application-level "already accepted"/"still in_flight" guard). `manual_contact_status` is the operator contact-lifecycle sidecar (`active`/`inactive`/`hold`); `inactive`/`hold` is a hard exact-email block enforced in [`outbound_campaign_gate.py`](../../src/origenlab_email_pipeline/outbound_campaign_gate.py) (wraps `candidate_export_gate.evaluate_export_eligibility` — does not fork gate logic), `active` is informational only and is **not** marketing consent. No `REFERENCES` into `contact_master`/`organization_master`/`lead_master` (rebuildable projections) — joined by normalized email/domain string only, same convention as `outreach_contact_state.lead_id`. CLI: [`scripts/campaigns/outbound_campaign_cli.py`](../../scripts/campaigns/outbound_campaign_cli.py). This is separate from, and does not replace, the Postgres `outbound.outbound_batch` export-audit mirror (`postgres_outbound_audit.py`). |

**Export eligibility (code policy, not a table):** [`candidate_export_gate.py`](../../src/origenlab_email_pipeline/candidate_export_gate.py) centralizes rules for **candidate** lists (`compute_next_marketing_recipients` / canonical lead export queue, archive-outreach queue audit/export path, and `export_marketing_from_contact_master`). The **`contact_master`** path enables **stricter marketing-email noise** than **`lead_master`** (same module, different flag). Sent-history blocking and current outreach memory are anchored to the current sender mailbox (`contacto@origenlab.cl`) plus operator sidecars; this policy surface complements operator tables above but is not DDL-owned schema. Read-only QA: [`scripts/qa/export_candidate_audit.py`](../../scripts/qa/export_candidate_audit.py). See [ARCHITECTURE § Cold outreach export eligibility](../ARCHITECTURE.md#m-eparch-export-gate), [RUNBOOK § Cold outreach export eligibility](../RUNBOOK.md#m-eprun-cold-export-gate), and [`../OUTBOUND_SOURCE_OF_TRUTH.md`](../OUTBOUND_SOURCE_OF_TRUTH.md).
