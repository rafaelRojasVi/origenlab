...
| **E — Observability + backup FD taxonomy** | Trusted backup WAL/SHM locking FD classification; sanitized OperationalError detail | **Complete** (PR [#397](https://github.com/rafaelRojasVi/origenlab/pull/397)) |
| **F — Incident regression pack** | Broader end-to-end incident-chain regressions | **Implemented in PR [#400](https://github.com/rafaelRojasVi/origenlab/pull/400)** |

**Merging PR-C (#394) does not authorize a cutover.** It only adds an explicit terminal path for an already-verified rollback. **PR-D (#395) does not authorize a cutover either** — it only adds persistent systemd enablement suppression (`systemctl --user disable`) for `origenlab-api.service` and `origenlab-api-health.timer` during maintenance, with exact restoration at terminal exits. **PR-E (#397) does not authorize a cutover** — it only adds fail-closed backup FD observation and sanitized SQLite `OperationalError` detail for `CREATE_CURRENT_BACKUP` / Online Backup API. **PR-F does not authorize a cutover** — it only adds synthetic end-to-end incident-chain regression coverage for A–E safety contracts. The abandoned maintenance ID `cutover20260719T163633Z` remains permanently non-resumable — it is rejected by `_require_auth` for **every** operation, including `--rollback-finalize` — and any future cutover requires a fresh MID and the full human approval boundary below. Waves 3B/3C remain blocked. Roadmap is A–F only (no PR-G).

### Incident-chain regression pack (PR-F)

PR-F adds `tests/test_sqlite_cutover_incident_regression_pack.py`: synthetic, temporary-directory regressions that chain A–E contracts across stage boundaries. Coverage includes permanent July 19 MID lockout before mutation; non-mutating preflight; `STOP_READERS` crash/restart and WSL restart suppression; FD quiescence and backup observation fail-closed gates; partial artifact barrier + SHM mail-resume block; smoke ownership/cleanup; smoke-fail → rollback → terminal `ABANDONED` (never `COMPLETED`); successful `COMPLETED` ordering with fail-before-terminal; durable-intent crash matrix; and the deferred `ambiguous_pre_exchange` residual for stale-journal-after-exchange. No production/runtime execution. PR-F is complete on `main` only after PR #400 merges. Merging PR-F does not authorize a cutover.
...
**Sealed-path binding (before any mutation).** Before any finalize intent, pause touch, or service mutation — and likewise before any rollback intent or physical exchange in `attempt_rollback_before_writers` — the sealed `PrivatePlanPaths` private-plan file is loaded and type-validated, and the production path, reports directory, journal path, and journal directory are bound **exactly** to that sealed plan (the reports directory used for pause operations is the sealed one, not an operator/settings re-derivation; on a real filesystem it must be a real, non-symlinked directory). A missing, malformed, non-object, symlinked, or path-inconsistent private plan — or an operator `--reports-dir` that does not match the sealed plan — is a sanitized **hard** `MANUAL_RECOVERY_REQUIRED` (path-free `sealed_plan_problem` token) that performs **no** file or service mutation. Absolute paths are never emitted in public evidence.

**Untrusted typed proof binding.** `approved_plan` and `rollback_intent` are treated as untrusted typed objects. `approved_plan` must be a dict with exact-integer `production_device`/`production_inode` and a non-empty `production_fingerprint`; `rollback_intent` must be a dict with non-empty `old_fingerprint`/`new_fingerprint`. The proof's old/restored fingerprint must equal **both** `approved_plan.production_fingerprint` and `rollback_intent.old_fingerprint`; its new fingerprint must equal **both** the journal `candidate_fingerprint` and `rollback_intent.new_fingerprint`. `staging_basename`, `pre_cutover_basename`, and `candidate_fingerprint` are **required, safe, non-empty and exactly equal to the proof values** (never conditional "only if present" checks). Terminal timestamps / post-mail fingerprint must be non-empty strings and the pre-maintenance service-policy fields must be **exact booleans** (a truthy `"false"`/`0`/`1` is refused). Every violation is sanitized `MANUAL_RECOVERY_REQUIRED`.

**API ownership is verified-success-after.** The temporary/health-check API start persists a durable start **intent with no ownership flag**, starts the unit externally, verifies **unambiguous** running, and only then captures durable process/unit evidence (`MainPID` + systemd `InvocationID` generation) and persists ownership. A crash after the start intent but before verified ownership leaves no owner record, so a later, externally started API at the same PID can never be *proven* owned — it is manual recovery and is never stopped. Reuse of an already-running API requires **proven** ownership (flag + live identity match). Manual recovery classifies API activity once, never stops an ambiguous or unproven process, persists a cleanup stop intent before stopping a proven-owned API, and retains ownership when the stopped state is uncertain.

**Timer + final convergence fail closed.** A structured `classify_health_timer_activity(...)` (known-active / known-inactive / ambiguous) backs `FilesystemAdapters.health_timer_activity()`; a systemctl invocation failure (127), unknown/unreachable text, `activating`/`deactivating`/`reloading`, or `failed` is **ambiguous → manual recovery**, never silently "inactive". Timer stop, timer start, and **final service convergence** all use this classifier (and `api_activity` for the API side); any ambiguous timer or API reading at convergence fails closed.

**Pre-exchange validation.** `attempt_rollback_before_writers` now enforces git / canonical-production / journal / sealed-private-plan bindings, a safe single-component `pre_cutover_basename`, a regular non-symlink capturable retained original, full `approved_plan` identity, and all proof source fields **before** writing `rollback_intent` or performing `RENAME_EXCHANGE`; `--expected-production-fingerprint` is bound to the current/new rollback side. No `int()`/`Path.with_name()`/schema failure can first occur after the exchange (the post-exchange proof build is pure serialization). After the exchange, `rollback_verified` is set **only** if the restored fingerprint, authoritative identity, and approved-plan invariants all pass; otherwise a conservative `rollback_identity_capture_failed` post-exchange lock is persisted. The deferred general `reconcile_atomic_swap_state` classifier is unchanged.

**Unknown manual-recovery truth.** When `list_writers` fails during manual recovery, mail and mirror pause presence are recorded **independently** as present / absent / **unknown** (`rollback_finalize_mail_pause_known` / `rollback_finalize_mirror_pause_known`) rather than trusting a stale absent/present boolean; sanitized evidence carries the per-writer state. Terminal coherence additionally **requires known, verified** pause states (and exact-boolean pre-maintenance policy), so an unknown/stale observation can never qualify as coherent `ABANDONED`.

**Proven API ownership at every stop.** Every API stop requires both an exact-boolean ownership flag (`smoke_started_api` / `rollback_finalize_started_api` / `rollback_finalize_owned_temp_api`) and live MainPID + InvocationID equality against the durable `rollback_finalize_api_owner` record. That owner object is untrusted: exact positive-integer MainPID and a non-empty string InvocationID are mandatory — missing InvocationID never degrades ownership to PID-only, and malformed fields fail closed without raising. At the pre-health recheck, ambiguity is tested first; a start is issued only from an unambiguously stopped state; late unproven or ambiguous activity is never adopted, reused, or stopped.

**Sealed reports directory is authoritative for observation too.** Pause mutations and pause/quiescence observation both bind to the sealed reports directory: `list_writers(..., reports_dir=sealed)`, `_assert_writers_quiesced`, manual recovery, resume verification, and pre-exchange rollback validation never touch pauses in one directory and verify another via settings. If pause-absence observation fails after a mail or mirror unlink, finalize enters manual recovery immediately, reasserts both sealed pauses, preserves historical resume intent/success truth, and never records ABANDONED.

**Pre-exchange validation (complete).** Before `rollback_intent` or `RENAME_EXCHANGE`, the complete sanitized `ApprovedPlan` validator binds production basename to the actual expected/canonical production path, requires `reports_active_basename == "current"`, requires a safe non-empty journal `backup_basename` bound exactly to the plan/sealed destination, binds backup/staging basenames **and exact (or canonical-equivalent) dest paths** so a same-basename/different-parent option cannot pass, binds mail/mirror basenames to the sealed pause paths, binds approved production device/inode to the sealed private-plan identity, compares backup/staging parent device/inode to live parent captures, and requires non-negative exact-integer capacity fields. The same validator runs again during finalize/idempotency. Both exchange members are authoritatively captured before intent: the retained original is bound to the approved-plan identity and the current candidate to the ATOMIC_SWAP journal identity; typed candidate/retained device/inode are persisted in `rollback_intent`. After durable intent, **both** members are recaptured and compared (device/inode + fingerprint) to those approved values before `RENAME_EXCHANGE`. Drift leaves the durable lifecycle lock in place with no exchange. The deferred `reconcile_atomic_swap_state` classifier is unchanged.

**Presence locks + role-bound ownership.** Lifecycle lockout uses **presence** (`is not None`), not truthiness. Finalize/service intents are exact typed objects (action allowlist, matching MID, non-empty timestamps / exact phase / exact `owned_temp`); empty, truncated, unknown-action, or wrong-typed intents are sanitized manual recovery. An unresolved service intent is never overwritten with an unrelated action — known crash intents proceed only through action-specific observed-state reconciliation. Ownership evidence persists `role` + MainPID + InvocationID + maintenance ID; a stop requires exactly one exact-True role flag, matching owner role + MID, and live PID/InvocationID equality. Mixed/contradictory role flags are ambiguous and stop nothing; legacy smoke ownership without role-bound evidence is unproven. Stale roles clear only after an unambiguously stopped API.

**Fail-closed PR-C booleans.** Every PR-C boolean used for locking, ownership, writer-PoNR, recovery, or terminal decisions must be an exact `bool` (rejects `0`/`1`/`""`/`"false"`/`[]`/`{}`). Malformed values lock ordinary progression and route finalize to manual recovery before mutation. `attempt_rollback` and fresh finalize require `writer_resume_started` and `writers_resumed` to be exact `False`. Terminal coherence requires `rollback_identity_capture_failed is False` and exact types for all applicable terminal booleans, validated before committing/returning successful `ABANDONED`.

**Complete proof and terminal typing.** Fresh finalize requires stage `ATOMIC_SWAP`; terminal idempotency requires stage `ABANDONED`. Coherent terminal `ABANDONED` requires a structurally valid typed `rollback_sidecar_proof` (exact schema/timestamp/presence/ambiguity/device/inode); an ambiguous captured member or a list/string/non-empty malformed object never qualifies — terminal retries produce sanitized `MANUAL_RECOVERY_REQUIRED`, never `already_finalized=true`. `rollback_intent.pre_cutover_basename` is bound to the safe journal/proof basename. `rollback_verified`, `exchange_completed`, and `old_production_retained` are exact booleans; `rollback_finalized_at_utc` and `rollback_finalize_post_mail_fingerprint` are non-empty strings (not truthiness).
...
- `tests/test_sqlite_cutover_backup_fd_observability.py` — PR-E FD taxonomy, observation lifecycle, OperationalError matrix
- `tests/test_sqlite_cutover_incident_regression_pack.py` — PR-F end-to-end incident-chain regressions (introduced by PR #400)
