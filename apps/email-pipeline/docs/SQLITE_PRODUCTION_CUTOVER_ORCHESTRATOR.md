...
**Crash-safe finalization order.** (1) `--apply` + full approval + **exact repository HEAD** match; (2) strict terminal-consistency check (idempotent only on a *coherent* `ABANDONED` record; any mixed state → `MANUAL_RECOVERY_REQUIRED`); (3) typed `rollback_proof` schema validation, requiring approved-plan device/inode; (4) durably write `rollback_finalize_intent` before any mutation; (5) **reassert both writer pauses and prove quiescence (pauses + locks + FDs) BEFORE any mutable-fingerprint gate**, so an interrupted finalize whose writers legitimately resumed is re-paused and recovered instead of rejected on content drift; (6) immutable identity bindings (device/inode + approved plan) and candidate-not-production; the pristine size/mtime fingerprint is required only while no writer-pause removal was ever recorded (once mail resumed, identity + `quick_check` carry safety); (7) verify WAL/SHM lifecycle against `rollback_sidecar_proof` **before** touching services — an appeared / disappeared / drifted / candidate / ambiguous sidecar fails closed (never delete/chmod); (8) restore the original main/WAL/SHM using PR-B's exact device/inode-bound logic rebound to the **original** (now-live) inode; (9) reconcile services against the captured **pre-maintenance policy**, stopping **only** an API this MID owns (retain ownership + `MANUAL_RECOVERY_REQUIRED` on any unverified stop), and verifying timer stop/start outcomes; (10) verify health against the restored original via the bounded loopback getter, using an **owned temporary API** (started/verified/stopped) when the API was inactive pre-maintenance; (11) recapture API-open sidecars through hardened `lstat`/open/`fstat`; (12) converge to and verify the pre-maintenance service state; (13) verify main + every present sidecar writable; (14) resume mail then mirror with intent-before/success-after and observation gates; (15) only then durably set `abandoned=true`, `rollback_finalized=true` (+ timestamp), clear `rollback_finalize_intent`, `stage=ABANDONED`, fsync the journal dir, leaving `completed=false` and reporting `cutover_succeeded=false`, `soak_eligible=false`, `waves_unblocked=false`.

**Immutable vs mutable identity.** Device/inode and the approved-plan original identity are **immutable bindings** checked on every entry/retry. The original size/mtime **fingerprint** is mutable once writers legitimately resumed: finalize records `rollback_finalize_post_mail_fingerprint` at the instant mail may write, and a retry after real mail activity relies on immutable identity + `quick_check` rather than demanding the pristine fingerprint — so recovery never rejects a legitimately-changed original while leaving writers live.

**Sidecar lifecycle.** `RENAME_EXCHANGE` moves only the main file, so `attempt_rollback_before_writers` records `rollback_sidecar_proof` (exact WAL/SHM identity or explicit absence) immediately after the physical rollback.

| Boundary | WAL/SHM expectation |
|----------|----------------------|
| after rollback (proof captured) | identity snapshot or explicit absence |
| before API start | must equal proof **and** be an original barrier-applied companion; else fail closed |
| candidate-created / appeared / disappeared / drifted / ambiguous | `MANUAL_RECOVERY_REQUIRED` (never delete/chmod) |
| after restored-original API open | new sidecars legitimate; hardened-recaptured, then writable-verified |

**Service ownership / pre-maintenance policy.** `STOP_READERS` durably captures `pre_maintenance_api_active` / `pre_maintenance_health_timer_active` before stopping anything. Finalize starts **only** services that were active pre-maintenance, treats an unexpectedly-active unowned API as ambiguous (`MANUAL_RECOVERY_REQUIRED`), never clears ownership on an unverified stop, verifies every timer stop/start, and asserts the final service state equals the captured policy. A legacy journal missing the capture fails closed (backward-compatible refusal).

**Failure / retry behavior.** Each externally visible mutation persists intent before and success after (mail/mirror each get `*_resume_intent`, `*_pause_absent`, `*_observed_ok`), preserving atomic journal replacement + file/dir fsync. A crash before the terminal write leaves `rollback_finalize_intent` set, so the normal cutover path stays locked and re-running `--rollback-finalize` reconciles actual pause/service/identity state and safely finishes `ABANDONED` — it never restarts the normal path. **Historical** resume truth (`*_resumed`) is set once and **never reset**; only **current** pause state (`*_pause_absent`, `rollback_finalize_writers_repaused`) changes on re-pause. A partial writer resume (observation fails) fails safe by reasserting **both** pauses, verifying re-pause + quiescence, preserving cleanup/journal-write failures in evidence, and returning sanitized `MANUAL_RECOVERY_REQUIRED` (never `ABANDONED`/`COMPLETED`/success). Sanitized errors contain no token, secret, response body, or absolute production path.

**Command / approval.** `orchestrate_sqlite_production_cutover.py --rollback-finalize` **requires `--apply`**, the full production approval contract (`--confirm-production-cutover`, exact MID / path / fingerprint / `--expected-main-sha`, actual repository HEAD match, `--approve-swap`), and is mutually exclusive with `--abort-before-swap`, `--rollback-before-writers`, and any non-default `--stage`. On a valid rolled-back journal it executes/reconciles; on a **coherent** already-`ABANDONED` journal it returns a safe idempotent "already finalized" status; on `COMPLETED`, an in-progress cutover, a post-PoNR journal, or a **mixed** terminal record it refuses / requires manual reconciliation. Status output distinguishes `ABANDONED` from success and states that operations run against the restored original.
