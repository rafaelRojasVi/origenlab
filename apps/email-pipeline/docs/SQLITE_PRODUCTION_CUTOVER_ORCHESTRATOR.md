...
**Pre-exchange validation (complete).** Before `rollback_intent` or `RENAME_EXCHANGE`, the authoritative fstat capture of the retained original is compared to the approved-plan production device/inode (replacement/drift refuses before exchange); API and health-timer activity are validated with the structured fail-closed classifiers (not binary `service_state()`); command failure, ambiguity, reloading, activating, or deactivating means no exchange. Any durable `rollback_intent` is itself a lifecycle lock for ordinary stages, abort, and a second rollback — even if the later post-exchange journal write failed. The deferred `reconcile_atomic_swap_state` classifier is unchanged.

**Complete proof and terminal typing.** Fresh finalize requires stage `ATOMIC_SWAP`; terminal idempotency requires stage `ABANDONED`. The complete `ApprovedPlan` schema is validated with exact types and bound to journal/options (MID, SHA, production identity/basename, staging basename, pause basenames). `rollback_intent.pre_cutover_basename` is bound to the safe journal/proof basename. `rollback_verified`, `exchange_completed`, and `old_production_retained` are exact booleans; `rollback_finalized_at_utc` and `rollback_finalize_post_mail_fingerprint` are non-empty strings (not truthiness).
