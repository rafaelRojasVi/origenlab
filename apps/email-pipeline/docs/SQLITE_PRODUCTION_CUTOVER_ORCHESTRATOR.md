...
**Pre-exchange validation (complete).** Before `rollback_intent` or `RENAME_EXCHANGE`, the complete sanitized `ApprovedPlan` validator binds production basename to the actual expected/canonical production path, requires `reports_active_basename == "current"`, binds backup/staging basenames and sealed dest paths (and opts where supplied), binds mail/mirror basenames to the sealed pause paths, binds approved production device/inode to the sealed private-plan identity, compares backup/staging parent device/inode to live parent captures, and requires non-negative exact-integer capacity fields. The same validator runs again during finalize/idempotency. The authoritative fstat capture of the retained original is compared to the approved-plan production device/inode (replacement/drift refuses before intent); API and health-timer activity are validated with the structured fail-closed classifiers (not binary `service_state()`). After durable `rollback_intent` is written, **both** exchange members are immediately recaptured and revalidated against the already approved identity/fingerprint values — only then is `RENAME_EXCHANGE` called. Drift, replacement, symlink, FIFO, capture failure, or content change leaves the durable lifecycle lock in place, performs no exchange, and requires manual recovery; the intent is never cleared merely to permit a retry. The deferred `reconcile_atomic_swap_state` classifier is unchanged.

**Presence locks + role-specific ownership.** Lifecycle lockout uses **presence** (`is not None`), not truthiness: an empty dict, list, string, or otherwise malformed `rollback_intent` / finalize / service intent still locks ordinary stages, abort, and a second rollback; malformed intent state routes finalize to sanitized manual recovery. Every ownership flag used as a decision gate must be an exact boolean — one role never substitutes for another. Temporary API stops require `rollback_finalize_owned_temp_api is True` specifically plus matching live MainPID and InvocationID; general reconcile may stop only when an explicitly enumerated appropriate role is proven.
