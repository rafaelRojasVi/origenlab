...
select is(
  (select count(*)::int from ol_fk), 102,
  'the seven application schemas declare 102 foreign keys');
select is(
  (select count(*)::int from ol_fk_coverage where coverage = 'full'), 81,
  '81 foreign keys are covered by an unconditional index');
select is(
  (select count(*)::int from ol_fk_coverage where coverage = 'implied-partial'), 21,
  '21 foreign keys are covered by a partial index whose predicate the lookup implies');
