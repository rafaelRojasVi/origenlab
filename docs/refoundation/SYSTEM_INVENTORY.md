...
> **Scope: V1.** This inventory maps the running V1 system, which is what the
> migration reads. The V2 target model is owned by [`../DOMAIN.md`](../DOMAIN.md)
> and [`../ARCHITECTURE.md`](../ARCHITECTURE.md); what is actually built is in
> [`../STATUS.md`](../STATUS.md).
...
| `supabase/` | **V2 durable core** — the accepted replacement for the V1 `commercial.*` Postgres boundary (decision 2026-09-05) | Nothing yet — 7 private schemas and 33 tables exist with **zero rows and zero consumers** | Nothing | None — no application code reads or writes it; no hosted project exists | PostgreSQL 17 via the Supabase CLI, local container only; CI `.github/workflows/supabase.yml` | **foundation only** — Migration Slice 0 local portion done, every hosted gate open. Canonical docs are [`../README.md`](../README.md), **not** this inventory; build state in [`../STATUS.md`](../STATUS.md) |
