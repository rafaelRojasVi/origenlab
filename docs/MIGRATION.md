...
| 10 | **No secret or legacy service-role key** is present in dashboard or web runtime configuration or in source control, and none is present in API or worker configuration as an application-data or database credential. The only permitted server-side use is a current `sb_secret_...` key for the Storage API — a use restriction, because that key resolves to `service_role` and is not Storage-scoped; where the API and the worker both hold one, the two keys are distinct, and no legacy JWT `service_role` key is configured anywhere ([`ARCHITECTURE.md`](ARCHITECTURE.md) §6.4, §7) |
